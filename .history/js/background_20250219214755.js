 ...
 */
function drawVague(ctx,canvas,steep){
    ...
    if(ctx instanceof CanvasRenderingContext2D){
        ...
        for(let t = -200;t < window.innerHeight+200;t+= 100){
            ...
            for(let i = 0 ;i < window.innerWidth;i+=5){
                ...
                ctx.beginPath();
                ctx.moveTo(x,y)
                ctx.fillStyle=`rgb(${color+(X*0.05)},${color+50},${color+50+(X*0.05)})`;
        ...
        requestAnimationFrame(() => drawVague(ctx, canvas,steep));
